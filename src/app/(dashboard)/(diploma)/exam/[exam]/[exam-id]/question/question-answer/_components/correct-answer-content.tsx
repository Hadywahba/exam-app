...
export default function CorrectAnswerContent({
  ...
  // functions
  const goToExam = () => {
    router.push(`/exam/${examContentId}`);
  };

  const resetExam = () => {
    router.push(`/exam/${examContentId}/${examId}/question`);
  };

  // Effect
  useEffect(() => {
    localStorage.removeItem(`currentQuestion_${question?.questions[0]?.text}`);
    setCurrent(0);
      ...
      <section>
        <div className="mb-4 flex w-full flex-col gap-2 sm:flex sm:flex-row sm:items-center sm:justify-between sm:gap-0">
          <p className="text-xs font-normal text-gray-500 sm:text-sm">
            {examTitle} - {answers?.submission?.examTitle}
          </p>
          <p className="text-xs font-normal text-gray-500 sm:text-sm">
            Question{' '}
