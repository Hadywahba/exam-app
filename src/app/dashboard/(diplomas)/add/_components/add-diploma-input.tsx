...
import DiplomaButtonsActions from '../../[id]/edit/_components/diploma-buttons-actions';
import TitleField from '../../[id]/edit/_components/title-field';
import DescriptionField from '../../[id]/edit/_components/description-field';
import {
  AddFormFields,
  addschema,
} from '@/lib/schemas/admin-dashboard/add-diploma.schema';
import { UseAddsDiplomas } from '../_hooks/add-diploma';

export default function AddDiplomaInput() {
  // Hook
  const { UploadImages, isPending: isUploading } = UseUpload();

  const { AddDiplomass, AddErrorDiplom, AddIsPendingDiplom } =
    UseAddsDiplomas();
  ...
  const onsubmit: SubmitHandler<AddFormFields> = (data) => {
    AddDiplomass(data);
  };
  return (
      ...
      <footer className="flex flex-col gap-4 bg-white">
        ...
        <section className="flex flex-col gap-3 p-4">
          ...
          <Input
            ...
            type="file"
            accept="image/*"
            disabled={isUploading || AddIsPendingDiplom}
