...
import { DeleteDiplomas } from '../_hooks/delete-diploma';
export default function DeleteAdminModal({
  ...
  // Mutation
  const { deleteDiplom, deleteErrorDiplom, deleteIsPendingDiplom } =
    DeleteDiplomas(diplomaId);
  return (
    <Modal
      isOpen={openModal}
    ...
    >
      ...
      {/* Error section */}
      <div>
        <SubmitError errors={deleteErrorDiplom} />
      </div>
      ...
      <div className="flex w-full items-center justify-center gap-6 border-t-[.0625rem] border-gray-200 bg-gray-50 p-6">
        ...
        <Button
          ...
          variant="destructive"
          className="w-[13.5625rem]"
          disabled={deleteIsPendingDiplom}
        >
          {deleteIsPendingDiplom ? 'Loading...' : 'Yes, delete'}
        </Button>
      </div>
    </Modal>
  );
